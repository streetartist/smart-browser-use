use browser_use::{browser::{ConnectionOptions, LaunchOptions}, mcp::BrowserServer};
...
#[derive(Parser, Clone)]
#[command(name = "smart-browser-use")]
#[command(version)]
...
struct Cli {
    ...
    /// Log file path for stdio mode (default: smart-browser-use-mcp.log)
    #[arg(long, default_value = "smart-browser-use-mcp.log")]
    log_file: String,
}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut options = LaunchOptions { headless: !cli.headed, ..Default::default() };

    if let Some(ref path) = cli.executable_path {
        options = options.chrome_path(PathBuf::from(path));
    }

    if let Some(ref dir) = cli.user_data_dir {
        options = options.user_data_dir(PathBuf::from(dir));
    }
    ...
    match cli.transport {
        Transport::Stdio => {
            ...
            let service = create_browser_server(&cli, options.clone())?;
        ...
        Transport::Sse => {
            ...
            let cli_template = cli.clone();
            let _cancellation_token = sse_server.with_service(move || {
                create_browser_server(&cli_template, options.clone()).expect("Failed to create browser server")
            });
        ...
        Transport::Http => {
            ...
            let service_factory = move || {
                create_browser_server(&cli_template, options.clone())
...
fn create_browser_server(cli: &Cli, options: LaunchOptions) -> Result<BrowserServer, String> {
    if let Some(ws_url) = &cli.ws_endpoint {
        BrowserServer::with_connection(ConnectionOptions::new(ws_url.clone()))
            .map_err(|e| format!("Failed to connect browser server: {}", e))
    } else {
        BrowserServer::with_options(options).map_err(|e| format!("Failed to create browser server: {}", e))
    }
}
